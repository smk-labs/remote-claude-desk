...
| Text pastes but an image never does, using a native client | xrdp 0.9.24's clipboard channel hands over a BMP whose header declares more bytes than arrive. ImageMagick refuses it with "length and filesize do not match", so every app pasting it gets nothing | `desk-tunnel` starts the SSH clipboard bridge, which carries images itself. Set `cliprdr=false` in xrdp.ini so the channel and the bridge are not both claiming the X CLIPBOARD selection |
