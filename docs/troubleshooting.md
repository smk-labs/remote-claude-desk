...
| The window opens, then the pointer becomes a spinner and macOS calls the client Not Responding | The server ran out of CPU. Something on it is holding a whole core, or `DESK_SIZE` is more than it can encode: xrdp compresses every frame in software | `desk-doctor` now names both. It reports any process that has averaged 80% of a core, and warns when the size is over 2.4 megapixels on a 2 core box |
