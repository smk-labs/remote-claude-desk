...
| The client quits a few seconds after connecting. The log says `ERRINFO_LOGOFF_BY_USER` | Nobody logged off. A session outlived the sesman that tracked it, so every new connect starts a second session, which dies at once | Let `desk-tunnel` heal it: it kills any xrdp Xorg older than sesman before it forwards the port, and does it again on every run the watcher triggers once the LaunchAgent is installed. If it recurs, install the reaper from [../server/README.md](../server/README.md) |
...
| Having to run `desk-tunnel` before you can work | It does its job and exits by design: it holds nothing open, the SSH master does | `desk-tunnel --install` writes a LaunchAgent per machine that runs `--watch` at login and restarts it whenever it exits. Every step in it is idempotent, so a re-run on a healthy setup is three cheap checks, and a re-run after a wifi drop is the repair |
| Windows App says the PC is offline, but the box is up and `ssh` works | Nothing is listening on the forwarded port. The master was dropped by a sleep, a wifi handoff or a VPN switch, and until 2026-09-05 the agent only looked every five minutes | `desk-doctor` now has a Tunnel section that says exactly this, and `desk-tunnel --watch` closes the window to about ten seconds |
| The session drops on its own every half hour or so, and reconnecting works | The watcher was rebuilding a healthy tunnel on one slow probe. 173 times in four days, and 57% of the rebuilds succeeded inside a minute | Fixed 2026-09-09: three failed probes are needed, with an eight second ceiling each. If it comes back, the log names it: `grep 'failed 3 probes' ~/.cache/remote-claude-desk/tunnel-agent.log` |
| The clipboard stops crossing while everything else looks fine | The bridge gave up. Before 2026-09-09 a single failed round trip ended it for good; now it retries ten times over nine minutes and the watcher restarts it within a minute of it dying | `tail ~/.cache/remote-claude-desk/clip.log`. Every run of the watcher ends with a line saying why, so `grep 'watch: run for' ~/.cache/remote-claude-desk/tunnel-agent.log` says what happened around it |
| It connects, you log in, and you are on the OTHER machine | Two machines on one Mac means two local ports. A forward that fails is silent, so a port already held by the other machine's master accepts the connection and everything reports healthy | `desk_forward` compares the listener's pid to this machine's master and refuses to continue when they differ, naming the pid that holds it |
